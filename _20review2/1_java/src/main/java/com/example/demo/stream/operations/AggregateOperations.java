...
public class AggregateOperations {
    ...
    public static void main(String[] args) {
        ...
        DataPerson dataPersonMaxByComparator =  names.stream().max(new ComparePersonByName()).get();
        DataPerson dataPersonReversed =  names.stream().max(new ComparePersonByName().reversed()).get();
        System.out.println(dataPersonMaxByComparator);
        System.out.println(dataPersonReversed);
        System.out.println(names.stream().count());


        //reduces // define by users
        List<Integer> numbers = List.of(1,23,321,1);
